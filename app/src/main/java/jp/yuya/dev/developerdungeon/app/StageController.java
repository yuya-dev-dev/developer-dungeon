...
class StageController {
    ...
    private static final String TRAINING_ONE = "TRAINING-GIT-01";
    private static final String TRAINING_TWO = "TRAINING-GIT-02";
    private static final String TRAINING_THREE = "TRAINING-GIT-03";
    private static final List<CommandReference> COMMANDS = List.of(
            ...
            new CommandReference(3, "git diff --staged", "次のcommitへ含めるstage済みの変更を確認する"),
            new CommandReference(4, "git add <file>", "指定fileの変更を次のcommitへ含める"),
            new CommandReference(5, "git restore --staged <file>", "指定fileを次のcommit対象から外す"),
            new CommandReference(6, "git commit -m <message>", "stage済みの変更をmessage付きでlocal履歴へ記録する"),
            new CommandReference(7, "git commit -m restore-required-settings", "stage済みの復旧内容を固定messageでcommitする"),
            new CommandReference(8, "git log --oneline", "commit履歴を短い形式で確認する"),
            new CommandReference(9, "git show <commit-id>", "特定commitの内容を確認する"),
            new CommandReference(10, "git branch", "local branchの一覧と現在位置を確認する"),
            new CommandReference(11, "git switch <branch>", "作業するbranchを切り替える"),
            new CommandReference(12, "git switch -c <branch>", "新しいbranchを作成して切り替える"),
            new CommandReference(13, "git stash push", "未commitの変更を一時退避する"),
            new CommandReference(14, "git stash pop", "一時退避した変更を適用してstashから削除する"),
            new CommandReference(15, "git stash apply", "一時退避した変更を適用し、stashにも残す"),
            new CommandReference(16, "git stash drop", "適用済みのstashを削除する"),
            new CommandReference(17, "git revert --no-edit <commit-id>", "共有履歴を残したまま変更を打ち消す"),
            new CommandReference(18, "git revert --no-commit <commit-id>", "打ち消す変更をcommitせずにstageする"),
            new CommandReference(19, "git cherry-pick <commit-id>", "特定commitの変更を現在のbranchへ適用する"),
            new CommandReference(20, "git reset --hard <commit-id>", "未共有のbranchを指定commitの状態へ戻す"),
            new CommandReference(21, "git merge <branch>", "別branchの変更を現在のbranchへ統合する"),
            new CommandReference(22, "git commit --no-edit", "用意されたmessageで途中の操作を確定する"),
            new CommandReference(23, "git commit -a --no-edit", "追跡中の変更をstageして途中の操作を確定する"),
            new CommandReference(24, "git reflog", "HEADが過去に指していた履歴を確認する"),
            new CommandReference(25, "git branch <branch> <commit-id>", "指定commitを指すbranchを作成する"),
            new CommandReference(26, "git switch -c <branch> <commit-id>", "指定commitからbranchを作成して切り替える"));
    ...
    @GetMapping("/git/training") String trainingList() { return "redirect:/git/stages#training-stages"; }
    ...
    @GetMapping("/training/TRAINING-GIT-01") String trainingOne(Model model) { add(model, TRAINING_ONE, stages.open(TRAINING_ONE)); return "stage"; }
    @GetMapping("/training/TRAINING-GIT-02") String trainingTwo(Model model) { add(model, TRAINING_TWO, stages.open(TRAINING_TWO)); return "stage"; }
    @GetMapping("/training/TRAINING-GIT-03") String trainingThree(Model model) { add(model, TRAINING_THREE, stages.open(TRAINING_THREE)); return "stage"; }
    ...
    @PostMapping("/training/TRAINING-GIT-01/commands") String trainingOneCommand(@RequestParam String command, @RequestParam String requestId, Model model) { add(model, TRAINING_ONE, stages.execute(TRAINING_ONE, command, requestId)); return "stage"; }
    @PostMapping("/training/TRAINING-GIT-02/commands") String trainingTwoCommand(@RequestParam String command, @RequestParam String requestId, Model model) { add(model, TRAINING_TWO, stages.execute(TRAINING_TWO, command, requestId)); return "stage"; }
    @PostMapping("/training/TRAINING-GIT-03/commands") String trainingThreeCommand(@RequestParam String command, @RequestParam String requestId, Model model) { add(model, TRAINING_THREE, stages.execute(TRAINING_THREE, command, requestId)); return "stage"; }
    ...
    @PostMapping("/training/TRAINING-GIT-01/hint") String trainingOneHint(Model model) { add(model, TRAINING_ONE, stages.hint(TRAINING_ONE)); return "stage"; }
    @PostMapping("/training/TRAINING-GIT-02/hint") String trainingTwoHint(Model model) { add(model, TRAINING_TWO, stages.hint(TRAINING_TWO)); return "stage"; }
    @PostMapping("/training/TRAINING-GIT-03/hint") String trainingThreeHint(Model model) { add(model, TRAINING_THREE, stages.hint(TRAINING_THREE)); return "stage"; }
    ...
    @PostMapping("/training/TRAINING-GIT-01/reset") String trainingOneReset(Model model) { add(model, TRAINING_ONE, stages.reset(TRAINING_ONE)); return "stage"; }
    @PostMapping("/training/TRAINING-GIT-02/reset") String trainingTwoReset(Model model) { add(model, TRAINING_TWO, stages.reset(TRAINING_TWO)); return "stage"; }
    @PostMapping("/training/TRAINING-GIT-03/reset") String trainingThreeReset(Model model) { add(model, TRAINING_THREE, stages.reset(TRAINING_THREE)); return "stage"; }

    private void add(Model model, String stageKey, StageView view) {
        model.addAttribute("view", view);
        StageDefinition stage = stages.definition(stageKey);
        boolean training = stageKey.startsWith("TRAINING-GIT-");
        ...
        String prefix = training ? "/training/" : "/stages/";
        model.addAttribute("actions", new StageActions(prefix + stageKey + "/commands#stage-workspace",
                prefix + stageKey + "/hint#stage-sidebar-hint",
                prefix + stageKey + "/reset#stage-workspace",

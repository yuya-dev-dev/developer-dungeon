...
- 問題文と進捗はDeveloper Dungeon内で管理し、実装、compile、採点、動作確認、任意の設計レビューは利用者がVS Code等のlocal環境と外部のChatGPTを使って行う。
...
5. 採点、動作確認、設計レビューが必要な場合は、利用者が問題文と自分のcodeを外部のChatGPTへ渡して行う。
...
- AST解析、自動採点、唯一解判定
- AI APIによる自動レビュー
- 問題集側での採点、動作確認、固定の確認用`Main`、JUnit課題

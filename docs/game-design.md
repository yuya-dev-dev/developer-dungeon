...
|---|---|---|---|
| Chapter 0 Git基礎研修 | 新人研修から仮配属まで | 状態を確認し、commit内容と作業境界を選ぶ | 実装対象。初心者推奨・経験者はスキップ可能。3研修・25〜40分 |
...
| 独立問題集 | Javaクラス設計、PostgreSQL実習 | 物語を必須にせず、要求を読んで手を動かす | Javaは実装済み、PostgreSQLはPhase 1設計中 |
| 将来技術編 | コードレビュー、Docker・CI/CD、運用 | 技術領域をまたいで調査し、再発防止を合意する | 編ごとに再評価する |
| 全編の到達点 | 後輩を支援する立場 | 正解を教えるだけでなく、確認事項と安全な根拠を問い返す | 長期的なキャラクターアーク |

Chapter 0は文章中心の講義や事故対応の模倣にせず、通常の新人研修として実Gitを操作する任意の導入章とする。最初のcommit、commit対象の選別、作業branchの作成を3段階で体験する。具体的な許可操作、fixture、採点、画面導線は[`chapter-0-training.md`](chapter-0-training.md)を正本とする。
...
1. `/`のタイトル兼編選択画面でGit編を選ぶ。
2. `/git/stages`でGit基礎研修のTRAINING-GIT-01〜03、またはGit事故対応のSTAGE-GIT-01〜05を選ぶ。

タイトル画面は明るい日中のオフィスを背景に、作品名、短い説明、利用可能な編のcardを中心へ置く。この節の参照画像はGit編入口の視覚基準であり、初期のGit編MVPではGit編だけを表示した。その後に実装済みとなったJava編は固定cardとして追加済みである。SQL編cardはSQL編Phase 2で一覧routeが利用可能になる変更と同時に追加し、それ以前に無効cardだけを先行表示しない。Docker編の無効card、汎用編管理、共通Runner基盤も先行実装しない。
...
- 守るべき条件を明示する。
- 調査コマンドの候補が少ない。
- `TRAINING-GIT-01`〜`03`を該当させる。完了状態だけを表示し、star差を設けない。
...
Chapter 0を追加しても、Stage 1〜5の事故内容、技術目標、採点、安全境界、成長beatは維持する。Chapter 0終了時は通常の仮配属連絡までとし、Stage 1の事故連絡はChapter 1開始後に提示する。
...
- PostgreSQL実習問題集: Git編の物語や事故対応へ合わせず、架空の業務tableと要求からSQLを書き、実行結果またはDB状態を確認する独立した問題集とする。上位brandと編選択は共有するが、会話、人物、スター、成長beatを必須にしない。具体仕様は[`sql-practice.md`](sql-practice.md)を正本とする。
- Docker・CI/CD障害対応編: 起動失敗、health check、network、volume、buildやdeployの失敗を調査し、運用担当と復旧手順と再発防止を合意する。

Git編の将来章で共通利用するのは、世界観、登場人物の役割、主力サービス、チケット形式、調査・実行・確認・振り返りのゲームループ、主人公の責任拡大である。Java問題集とPostgreSQL実習問題集にはこの物語構造を必須にせず、上位brand、編選択、基本的な視覚規則だけを共有する。現在のGit code、DB、Runnerを将来編用に汎用化しない。
...
- Chapter 2、Git編Finale、Javaコードレビュー編、Docker・CI/CD編の具体的なシナリオ、技術境界、章順
